...
`这篇博客 <https://saltigavv.net/articles/file-locks/>`_ 讲了 File locks。
...
对某个范围内连续的页进行上锁，确保该页不会被系统换出。
主要应用场景为实时算法和高安全性数据处理。实时算法需要统计任务调度等性能瓶颈带来的开销，而换页则是具有很高不确定性的机制。
实时算法测试可以将全部页面算法，同时使用一些 dummy 写操作来避免页的强制缺失。fork 调用会准备 COW 机制
