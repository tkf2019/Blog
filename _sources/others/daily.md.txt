...
## Userspace Bypass: Accelerating Syscall-intensive Applications (OSDI '23)

优化 Syscall 开销的传统思路：将数据处理和 driver 放一起（用户态驱动）、提供诸如 io_uring 的编程接口。本文的设计综合考虑性能和兼容性，UB （Userspace Bypass）在不需要修改或重新构建已有应用的前提下，将两个系统调用之间的代码也放入内核执行。

实验思路：运行 micro-benchmark 和真实应用 Redis 和 Nginx （I/O 密集型），运行平台为 VM 和物理机器，KPTI 开启的情况下性能优势更明显；对比其他优化方法：io_uring 、F-Stack DPDK 、eBPF ，性能没有明显优势，但是不需要对应用进行任何修改。
